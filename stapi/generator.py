...
class Generator:
        ...
        for line in lines_list:
            if line.startswith("  ") and line[2] != " ":
                ...
                if prop in loaded_yaml["properties"]:
                    if (
                        "required" in loaded_yaml["properties"][prop]
                        and loaded_yaml["properties"][prop]["required"]
                    ):
    ...
    - name: apiKey
      ...
      description: API key
"""
                index_of_the_interesting_part = read_file.find(string_to_find) + len(
                    string_to_find
                )
                ...
                searchable_attributes[camelCase[i]] = (
                    "title" if "title" in interesting_part[:where_to_stop] else "name"
                )
...
"""
        ...
        def search(self, searchCriteria):
            url_to_post = self.url + "/api/v1/rest/"""
                + camelCase[i]
                + """/search?pageNumber=" + str(searchCriteria.pageNumber) + "&" + str(searchCriteria.pageSize)
    ...
    def search_criteria():
        ...
        for i in range(len(snake_case)):
            ...
            output_code += (
                ...
                + "\n"
                + "    def __init__(self, pageNumber, pageSize, sort"
                + named_arguments
            )
            output_code += (
                "\n        super("
                + PascalCase[i]
                + "SearchCriteria, self).__init__(pageNumber, pageSize, sort)\n"
            )
